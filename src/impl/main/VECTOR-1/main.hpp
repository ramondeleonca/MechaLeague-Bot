...
namespace ArmSetpoints {
  constexpr ArmSetpoint PICKUP = {30, 90};
  constexpr ArmSetpoint STOW = {150, 90};
  constexpr ArmSetpoint SCORE = {60, 90};
}
...
void loop() {
    ...
    if (ps5.isConnected()) {
        ...
        float rotSpeed = (((float)ps5.L2Value() - (float)ps5.R2Value()) / 127.0) * 2; // Radians per second
        ...
        float climberSpeed = ((float)ps5.RStickX() / 100) * 0.5; // Max Voltage used to calculate vbat percentage
        ...
        // lowerJoint.write(map(ps5.RStickY(), -127, 127, 0, 180));
